...
class CEOPastActionStoreListActivity : AppCompatActivity() {
    val gson = Gson()
    private var ceoActionAdapter: CEOPastActionStoreListAdapter? = null
    lateinit var pastActionDataCEO: CEOActionQuery.Ceo
    ...
    private fun initialise() {
        ...
        pastActionDataCEO = ActionsFragment.actionDataCEO
        ...
        filter_icon.setOnClickListener {
            ...
            StorePrefData.isFromCEOPastActionStore = true
    ...
    private fun setData() {
        if (pastActionDataCEO.actions?.stores!!.isNotEmpty()) {
            no_past_action_linear.visibility = View.GONE
            past_action_rv.visibility = View.VISIBLE
            ceoActionAdapter =
                CEOPastActionStoreListAdapter(this, pastActionDataCEO.actions!!.stores)
        ...
        ceoActionAdapter?.setOnItemCeoStoreClickListener(object :
            CEOPastActionStoreListAdapter.OnItemClickListener {
    ...
    private fun callDetailView(position: Int) {
        Logger.info("Navigation to CEO Action List clicked","Past Action Store List")
        val intent = Intent(this, CEOPastActionListActivity::class.java)
    ...
    private fun callCEOAction() {
        StorePrefData.isFromCEOPastActionStore = false
        ...
        lifecycleScope.launchWhenResumed {
           val areaCode = dbHelper.getAllSelectedAreaList(true)
            ...
            val response = try {
                apolloClient(this@CEOPastActionStoreListActivity).query(CEOActionQuery(areaCode.toInput(),
                    stateCode.toInput(),
            ...
            if (response.data?.ceo != null) {
                progressDialog.dismissProgressDialog()
                pastActionDataCEO = response.data?.ceo!!
                setData()
            }
        }
    }
